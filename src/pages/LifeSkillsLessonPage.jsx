// src/pages/LifeSkillsLessonPage.jsx
// Trang bài học Kỹ năng sống - Thiết kế cho trẻ em
import React, { useState } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { useAuth } from '../contexts/AuthContext';
import { useAudio } from '../contexts/AudioContext';
import { getLifeSkillsLesson } from '../data/lifeskillsLessons';
import { ArrowLeft, Home, Star, Heart, CheckCircle, XCircle, Sparkles, RotateCcw, Lightbulb, ChevronRight } from 'lucide-react';

// Confetti animation
const Confetti = ({ show }) => {
  if (!show) return null;
  const colors = ['#14b8a6', '#06b6d4', '#22c55e', '#eab308', '#f97316', '#ec4899'];
  return (
...
const getLevelColor = (level) => {
  const colors = {
    ...
    4: 'from-teal-500 to-green-500',
  };
  return colors[level] || 'from-teal-500 to-cyan-500';
};

export default function LifeSkillsLessonPage() {
  const { lessonId } = useParams();
  const navigate = useNavigate();
  const { completeLesson } = useAuth();
  const { playSound } = useAudio();

  const lesson = getLifeSkillsLesson(lessonId);
  ...
  if (!lesson) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-gradient-to-b from-teal-100 to-cyan-100">
        <div className="text-8xl mb-4">🌱</div>
        <p className="text-xl text-gray-600 mb-4">Bài học chưa sẵn sàng</p>
        <button onClick={() => navigate(-1)} className="px-6 py-3 bg-teal-500 text-white rounded-xl font-bold">
          Quay lại
        </button>
      </div>
    );
    ...
    setTimeout(() => {
      if (currentQ < questions.length - 1 && lives > (correct ? 0 : 1)) {
        ...
        if (percentage >= 60) {
          ...
          playSound('complete');
          completeLesson('lifeskills', lessonId, percentage);
        }
      }
    }, 2000);
  };
      ...
      <div className="min-h-screen bg-gradient-to-b from-teal-50 to-cyan-50">
        ...
        <div className={`bg-gradient-to-br ${color} p-6 pb-12 rounded-b-[40px]`}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/80">Kỹ năng sống - Cấp độ {lesson.level}</span>
          </div>
  ...
  if (stage === 'finished') {
    ...
    return (
      <div className={`min-h-screen flex flex-col items-center justify-center p-4 ${passed ? 'bg-gradient-to-b from-teal-400 to-cyan-500' : 'bg-gradient-to-b from-orange-400 to-red-500'}`}>
        ...
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/subject/lifeskills')} className={`flex-1 py-4 rounded-xl font-bold text-white flex items-center justify-center gap-2 ${passed ? 'bg-teal-500' : 'bg-cyan-500'}`}>
